...
async function loadProfiles() {
  if (sb) {
    const { data } = await sb.from('profiles').select('id, full_name, email, role, school_id, active');
    profilesById = {};
    allProfiles = data || [];
...
async function enviarListagemParaCooperativas(id) {
  ...
  for (const coopId of grupoIds) {
    ...
    if (enviouAuto) {
      await updateExcursion(id, { cooperativa_email_sent_at: new Date().toISOString(), cooperativa_email_last_error: null });
