...
async function confirmAssign() {
  ...
  if (jaAprovada) Object.assign(patch, {
  ...
  if (ok) {
    ...
    const precisaLista = jaAprovada && precisaListagemComNomesDocumentos(tripAtual);
    if (precisaLista) {
      const extra = {};
    ...
    if (jaAprovada) await notifyAssignedDrivers(tripAtual, 'Escala atualizada', 'Sua escala confirmada foi atualizada pelo Administrador.');
    toast((jaAprovada ? '✅ Motorista(s) atribuído(s) e escala publicada!' : '✅ Motorista(s) atribuído(s). A escala permanecerá interna até a aprovação administrativa.') + (voltas > 1 ? ` Número de viagens do veículo: ${String(voltas).padStart(2, '0')}.` : '') + (precisaLista ? ' A listagem de passageiros foi liberada.' : ''));
  }
  closeAssignModal();
}
