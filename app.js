...
const DEMO_APP_SETTINGS = { remetente_nome: 'Bora Lá - Excursões / Semed Nova Lima', remetente_email: 'excursao.semed@pnl.mg.gov.br', email_copia_setor: 'excursao.semed@pnl.mg.gov.br', drive_upload_url: '' };
...
async function loadAppSettings() {
  if (sb) {
    ...
    appSettings = { remetente_nome: map.remetente_nome || 'Bora Lá - Excursões / Semed Nova Lima', remetente_email: map.remetente_email || '', email_copia_setor: map.email_copia_setor || map.remetente_email || '', drive_upload_url: map.drive_upload_url || '' };
  }
}
...
function renderAgenda() {
  ...
  const linhas = filtered
    ...
    .map((a) => {
      ...
      if (isAgendaEditor && a.admin_decision !== 'reprovada' && a.situacao !== 'cancelada') {
        ...
        if (a.admin_decision === 'aprovada') acoes += ` <button onclick="openAssignModal('${a.id}')" class="text-indigo-600 hover:text-indigo-800 text-xs font-medium ml-2">Atribuir motorista(s)</button>`;
      ...
      if (role === 'admin') {
        const propostaBtn = `<button onclick="openValidacaoModal('${a.id}')" class="text-violet-600 hover:text-violet-800 text-xs font-medium ml-2" title="Visualizar a proposta pedagógica">📄 Proposta</button>`;
        const pcdView = (a.pca_count || 0) > 0
          ? `<button onclick="conferirListaPcd('${a.id}')" class="text-violet-600 hover:text-violet-800 text-xs font-medium ml-2">♿ Ver PCD</button>`
...
function renderPendencias() {
  ...
  if (ehAdmin) {
    ...
    container.innerHTML = [
      pendenciaBloco('Solicitações', 'Resultado da decisão administrativa.', decisoes.map((a) => pendenciaCard(a, { badge: a.admin_decision === 'aprovada' ? 'Aprovada' : 'Reprovada', tone: a.admin_decision === 'aprovada' ? 'emerald' : 'red', detail: a.admin_decision_reason ? `Motivo: ${a.admin_decision_reason}` : 'Sem observações.' })), '🚌', 'emerald'),
...
// A decisão administrativa é independente do parecer pedagógico. A aprovação libera
// a atribuição de motoristas, mas o motorista só verá a viagem depois da atribuição.
...
async function tentarEnviarEmailAutomatico(to, subject, text, cc = '') {
  ...
  try {
    ...
    const resp = await fetch(`${url}/functions/v1/send-cooperativa-email`, {
      ...
      body: JSON.stringify({ to, subject, text, cc: cc || undefined }),
    });
    const json = await resp.json().catch(() => ({}));
...
async function enviarListagemParaCooperativas(id) {
  ...
  for (const coopId of grupoIds) {
    ...
    const enviouAuto = coop && coop.email ? await tentarEnviarEmailAutomatico(coop.email, subject, body, copiaSetor) : false;
    if (enviouAuto) {
...
async function onEmailTipoChange() {
  ...
  document.getElementById('emailCorpo').value = buildListagemEmailBody(trip, ids, await getExcursionPassengersGrouped(trip.id), await listagemFilesForTrip(trip.id));
}
...
async function sendCooperativaEmail() {
  ...
  const copiaSetor = appSettings.email_copia_setor || appSettings.remetente_email || '';
  const enviou = await tentarEnviarEmailAutomatico(coop.email, assunto, corpo, copiaSetor);
  if (!enviou) {
...
async function confirmAssign() {
  ...
  if (!tripParaEscala || tripParaEscala.admin_decision !== 'aprovada') {
    toast('⚠️ Aprove a solicitação antes de atribuir motorista(s).', true); return;
  }
...
async function confirmSaveSettings() {
  ...
  appSettings = { remetente_nome: nome, remetente_email: email, email_copia_setor: email, drive_upload_url: driveUrl };
  if (!sb) saveDemoData();
