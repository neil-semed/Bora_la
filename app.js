...
async function refreshDadosDaTela(name) {
  ...
  try {
    await Promise.all([
      loadSchools(), loadAgenda(), loadProfiles(), loadVehicles(), loadDrivers(), loadKmLogs(), loadCooperativas()
    ]);
    ...
    .map((a) => {
      ...
      if (validadaCompletamente && role === 'admin' && ehSolicitacaoExterna) {
        if (a.requester_email) {
          const extLabel = a.passenger_access_status === 'aberto' ? '🔗 Reenviar link' : (a.passenger_access_status === 'enviado' ? '🔗 Cadastro enviado' : '🔗 Cadastro externo');
          const extBtn = `<button onclick="prepararLinkPassageiros('${a.id}')" class="text-indigo-600 hover:text-indigo-800 text-xs font-medium ml-2" title="Enviar link para a entidade cadastrar passageiros">${extLabel}</button>`;
  ...
  return `
    <div class="px-4 py-3 flex items-center justify-between gap-3">
      <div>
        ...
        <div class="text-xs text-slate-500">${hhmm(a.departure_time)} - ${hhmm(a.return_time)} • ${totalPax} passageiros</div>
      </div>
...
function gerarTokenAcessoPassageiros() {
  return (window.crypto && crypto.randomUUID) ? crypto.randomUUID() : 'pl_' + Date.now() + '_' + Math.random().toString(36).slice(2);
}
async function prepararLinkPassageiros(id) {
  ...
  const patch = { passenger_access_token: token, passenger_access_status: 'aberto', passenger_access_submitted_at: null };
  if (sb) {
    const { error } = await sb.from('excursions').update(patch).eq('id', id);
  ...
  toast('✉️ Link de cadastro preparado para envio por e-mail.');
}
...
async function confirmEnviarListagem() {
  ...
  if (foraDeNovaLima) {
    for (const did of driverIds) {
      ...
      for (const p of rows) {
        ...
        if ((nome && !documento) || (!nome && documento)) {
          const v = driverVehicle(did);
  ...
  if (foraDeNovaLima) {
    const totalPax = totalPassengers(trip);
    if (allRows.length !== totalPax) {
      toast(`⚠️ Para emitir a ATF, a listagem deve ter exatamente ${totalPax} passageiro(s), conforme a solicitação. Atualmente há ${allRows.length}.`, true);
      return;
    }
    if (allRows.some((p) => !p.nome || !p.documento)) {
      toast('⚠️ ATF obrigatória: informe nome e documento de todos os passageiros antes de enviar a listagem.', true);
      return;
    }
  }
...
function validateWizardStep(step) {
  ...
  if (step === 4) {
    ...
    if (wizardPcdList.some((p) => !p.nome_aluno.trim())) {
      toast('⚠️ Informe o nome de cada aluno PCD (ou reduza a quantidade).', true);
      return false;
    }
    const recorrencia = document.querySelector('input[name="wRecorrencia"]:checked').value;
