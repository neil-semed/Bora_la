...
function maybeRegisterMotoristaPwa() {
  ...
  const button = document.getElementById('installDriverAppBtn');
  const help = document.getElementById('installDriverAppHelp');
  window.addEventListener('beforeinstallprompt', (event) => {
    event.preventDefault();
    driverInstallPrompt = event;
    if (button) { button.disabled = false; button.textContent = '📲 Instalar App do Motorista'; }
  });
  window.addEventListener('appinstalled', () => {
    driverInstallPrompt = null;
    if (button) { button.disabled = true; button.textContent = '✅ App instalado'; }
    if (help) help.textContent = 'O app foi instalado na tela inicial do celular.';
  });
  button?.addEventListener('click', async () => {
    if (driverInstallPrompt) {
      driverInstallPrompt.prompt();
      await driverInstallPrompt.userChoice;
      driverInstallPrompt = null;
      return;
    }
    if (help) help.textContent = 'Se o botão de instalação não abrir, atualize esta página uma vez. No iPhone use Safari → Compartilhar → Adicionar à Tela de Início.';
  });
}
...
async function loadProfiles() {
  if (sb) {
    const { data } = await sb.from('profiles').select('id, full_name, email, role, school_id, driver_id, active');
    profilesById = {};
    allProfiles = data || [];
...
function showScreen(name, el) {
  ...
  const titulo = name === 'km' && currentUser?.role === 'admin'
...
function renderDashboardCharts(visible) {
  ...
  if (ctx1) {
    ...
    chartMesInstance = new Chart(ctx1, {
      ...
      options: { responsive: true, plugins: { legend: { display: false }, title: { display: true, text: 'Evolução de viagens', align: 'start', color: '#1e293b', font: { size: 14, weight: '600' } } }, scales: { y: { beginAtZero: true, ticks: { precision: 0, color: '#64748b' }, grid: { color: 'rgba(148,163,184,.16)' } }, x: { ticks: { color: '#64748b' }, grid: { display: false } } } },
    });
  }
  ...
  if (ctx2) {
    ...
    chartUnidadeInstance = new Chart(ctx2, {
      ...
      options: { responsive: true, cutout: '64%', plugins: { legend: { position: 'right', labels: { boxWidth: 10, boxHeight: 10, padding: 12, color: '#475569', font: { size: 11 } } }, title: { display: true, text: 'Distribuição por unidade', align: 'start', color: '#1e293b', font: { size: 14, weight: '600' } } } },
    });
  }
}
...
function filterAgenda() {
  ...
  // O motorista acompanha somente as viagens já confirmadas pelo Admin, inclusive
  // as de outros motoristas. Os botões operacionais seguem protegidos por escala.
  const base = currentUser?.role === 'motorista' ? agenda.filter(viagemConfirmadaParaMotorista) : getVisibleAgenda();
...
function renderAgenda() {
  ...
  const linhas = filtered
    ...
    .map((a) => {
      ...
      const outroMotorista = ehMotorista ? (a.driver_ids || []).find((id) => id !== currentUser.driverId) : null;
      const outro = outroMotorista ? drivers.find((d) => d.id === outroMotorista) : null;
      const outroVeiculo = outro ? driverVehicle(outro.id) : null;
    ...
    <div class="${ehMotorista ? 'driver-trip-card' : 'border'} rounded-lg overflow-hidden bg-white shadow-sm">
      ...
      <div class="p-4 space-y-2.5 text-sm">
        ...
        ${motoristaVinculado ? `<div class="flex items-center justify-between gap-2 rounded-lg bg-emerald-50 px-3 py-2"><span class="text-emerald-800 text-xs font-medium">Operação no veículo</span><span class="text-emerald-800 text-sm font-bold">Número de viagens: ${String(viagensIndicadas).padStart(2, '0')}</span></div>` : ''}
        <div class="flex items-center justify-between gap-2">
        ...
        ${ehMotorista ? `<div class="text-xs text-slate-600"><span class="text-slate-400">Solicitante:</span> ${escapeHtml(a.requester_name || schoolName(a.school_id) || '-')} ${a.requester_contact ? `• ${escapeHtml(a.requester_contact)}` : ''}</div>` : ''}
        ${ehMotorista && outro ? `<div class="text-xs text-slate-600 bg-slate-50 rounded p-2"><span class="text-slate-400">Viagem compartilhada:</span> ${escapeHtml(outro.name)}${outro.phone ? ` • ${escapeHtml(outro.phone)}` : ''}${outroVeiculo ? ` • ${escapeHtml(outroVeiculo.plate || '')}` : ''}</div>` : ''}
      </div>
...
function renderAgendaPorDataItem(a) {
  ...
  const outroId = currentUser?.role === 'motorista' ? (a.driver_ids || []).find((id) => id !== currentUser.driverId) : null;
  const outro = outroId ? drivers.find((d) => d.id === outroId) : null;
  const outroVeiculo = outro ? driverVehicle(outro.id) : null;
  ...
  return `
    <div class="driver-trip-card px-4 py-4 flex items-center justify-between gap-3 bg-white rounded-xl mb-3">
      <div>
        ...
        <div class="text-xs text-slate-500">${hhmm(a.departure_time)} - ${hhmm(a.return_time)} • ${totalPax} passageiros SEMED</div>
        <div class="text-xs text-slate-500 mt-1">${originAddress(a) || '-'} → ${a.destination_address || a.city || '-'}</div>
        ...
        ${currentUser?.role === 'motorista' ? `<div class="text-xs text-slate-600 mt-1">Solicitante: ${escapeHtml(a.requester_name || schoolName(a.school_id) || '-')}${a.requester_contact ? ` • ${escapeHtml(a.requester_contact)}` : ''}</div>` : ''}
        ${outro ? `<div class="text-xs text-slate-600 mt-1">Compartilhada com ${escapeHtml(outro.name)}${outro.phone ? ` • ${escapeHtml(outro.phone)}` : ''}${outroVeiculo?.plate ? ` • ${escapeHtml(outroVeiculo.plate)}` : ''}</div>` : ''}
      </div>
...
function renderAgendaGeralMotorista() {
  ...
  container.innerHTML = dias.length ? dias.map((data) => {
    ...
    return `<div class="border rounded-xl overflow-hidden"><div class="px-4 py-3 bg-slate-50 border-b flex justify-between"><strong class="capitalize">${rotulo}</strong><span class="text-xs text-slate-500">${porDia[data].length} viagem(ns)</span></div><div class="p-3">${porDia[data].map(renderAgendaPorDataItem).join('')}</div></div>`;
...
async function confirmCancel() {
  ...
  if (ok) {
    if (trip) await notifyAssignedDrivers(trip, 'Viagem cancelada', `A viagem para ${trip.destination || '-'} em ${trip.trip_date ? new Date(trip.trip_date + 'T00:00').toLocaleDateString('pt-BR') : '-'} foi cancelada. Motivo: ${motivo}`);
...
function validateWizardStep(step) {
  ...
  if (step === 2) {
    ...
    } else if (!document.getElementById('wDestino').value.trim()) {
      toast('⚠️ Informe o destino.', true);
      return false;
    }
  }
  ...
  if (step === 3) {
    if (!document.getElementById('wData').value || !document.getElementById('wHora').value) {
      toast('⚠️ Informe data e horário de saída.', true);
      return false;
    }
  }
  ...
  tbody.innerHTML = drivers.map((m) => {
    ...
    <tr class="hover:bg-slate-50">
      ...
      <td class="px-4 py-3 text-sm whitespace-nowrap">
        ...
        <button onclick="deleteDriverSafely('${m.id}')" class="ml-2 text-xs text-red-600 hover:text-red-800">Excluir</button>
      </td>
    </tr>`;
  }).join('');
}
