...
-- Este é um backfill de manutenção executado pelo SQL Editor; o gatilho é
-- desativado somente durante esta atualização e imediatamente reativado.
alter table public.excursions disable trigger trg_guard_excursion_update;
...
alter table public.excursions enable trigger trg_guard_excursion_update;
...
-- Escritas feitas por RPCs SECURITY DEFINER já autorizadas abaixo precisam passar
-- pelo mesmo gatilho sem fingir que o usuário é Admin. A marca é local à transação
-- e só é definida dentro das próprias RPCs depois de validar perfil e vínculo.
...
insert into public.app_settings(key, value)
values ('escala_emails', '')
on conflict (key) do nothing;

-- Regrava as confirmações da migration 023 com a marca interna acima. O agente
-- continua limitado à própria cooperativa e às modalidades habilitadas.
...
declare v excursions%rowtype; begin
  ...
  if not exists (select 1 from excursion_drivers ed join drivers d on d.id=ed.driver_id join cooperativas c on c.id=d.cooperativa_id where ed.excursion_id=v.id and d.cooperativa_id=public.current_cooperativa_id() and c.opera_atf) then raise exception 'Esta cooperativa não está vinculada ou não opera ATF para esta viagem.'; end if;
...
declare v excursions%rowtype; begin
  ...
  if not exists (select 1 from excursion_drivers ed join drivers d on d.id=ed.driver_id join cooperativas c on c.id=d.cooperativa_id where ed.excursion_id=v.id and d.cooperativa_id=public.current_cooperativa_id() and c.opera_pcd) then raise exception 'Esta cooperativa não está habilitada para transporte PCD nesta viagem.'; end if;
  perform set_config('app.bora_la_system_write','true',true);
