...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { email, password, full_name, phone, role, school_id, driver_id, setor_pedagogico, access_profile_id, cooperativa_id } = body || {};
    if (!email || !password || !role) {
      return json({ error: 'Preencha e-mail, senha e perfil.' }, 400);
    }
    if (!['admin', 'escola', 'pedagogia', 'motorista', 'operacional', 'agente_externo'].includes(role)) {
    ...
    if (role === 'agente_externo' && !cooperativa_id) {
      return json({ error: 'Selecione a cooperativa do agente externo.' }, 400);
    }
    ...
    const { error: profileErr } = await admin.from('profiles').upsert({
      ...
      cooperativa_id: role === 'agente_externo' ? cooperativa_id : null,
    });
