...
function renderAgenda() {
  ...
  if (ehMotorista) {
    ...
    // igual à Agenda Geral (renderAgendaGeralMotorista), só que aqui restrito
    // à MESMA data mostrada nesta tela (hoje, ou a próxima data com escala -
    // motoristaHojeDataExibida, já calculada acima por filterAgenda()). Sem
    // isso, "Agendas de Hoje" (a tela que o motorista vê ao logar) nunca
    // mostrava a van do MarkCarro - só a Agenda Geral mostrava.
    carregarAgendaHojeMarkCarro();
  }
...
// Mesma van do MarkCarro, mas pra tela "Agendas de Hoje" (screen "agenda", a tela que
// o motorista vê ao logar) - só a data exibida ali (hoje, ou a próxima com escala),
// não os próximos dias inteiros como na Agenda Geral.
async function carregarAgendaHojeMarkCarro() {
  const wrapMc = document.getElementById('agendaHojeMarkCarroWrap');
  const listaMc = document.getElementById('agendaHojeMarkCarro');
  const avisoMc = document.getElementById('agendaHojeMarkCarroAviso');
  if (!wrapMc || !listaMc) return;
  const minhaPlaca = placaDoMotoristaAtual();
  wrapMc.classList.toggle('hidden', !minhaPlaca);
  if (!minhaPlaca) return;

  const data = motoristaHojeDataExibida || fmtDate(new Date());
  try {
    const doMarkCarro = await buscarAgendaMarkCarro(data, data);
    avisoMc?.classList.add('hidden');
    const minhas = (doMarkCarro || [])
      .filter((l) => (l.placa || '').toUpperCase() === minhaPlaca.toUpperCase())
    ...
    listaMc.innerHTML = minhas.length ? minhas.map((l) => `
      <div class="border rounded-lg px-3 py-2 text-sm flex items-center justify-between gap-3">
        <div><strong>${hhmm(l.hora_saida)}</strong>${l.hora_retorno ? ' - ' + hhmm(l.hora_retorno) : ''}<span class="block text-slate-500">${l.detalhe || ''}</span></div>
        <span class="text-xs text-slate-500 shrink-0">${l.status || ''}</span>
      </div>
    `).join('') : '<p class="text-center text-xs text-slate-500 py-4">Nenhuma corrida do MarkCarro nessa data com essa placa.</p>';
  } catch (e) {
    console.error('Erro ao buscar agenda do MarkCarro (Agendas de Hoje):', e);
    avisoMc?.classList.remove('hidden');
    listaMc.innerHTML = '';
  }
}
