  ...
  const linhas = filtered
    ...
    .map((a) => {
      ...
      if (a.status === 'pending' && isAgendaEditor) {
        const validarHandler = role === 'pedagogia' ? 'pedagogyApprove' : 'adminValidateOccurrence';
        const validarTexto = role === 'pedagogia' ? 'Aprovar (Pedagogia)' : 'Validar esta data';
        acoes = `
          <button onclick="${validarHandler}('${a.id}')" class="text-emerald-600 hover:text-emerald-800 text-xs font-medium mr-3">${validarTexto}</button>
...
function renderPendencias() {
  ...
  if (ehAdmin) {
    ...
    container.innerHTML = [
      ...
      pendenciaBloco('Proposta aguardando da unidade', 'Pedido do Admin ou correção solicitada pela Pedagogia.', reenvio.map((a) => pendenciaCard(a, { badge: a.doc_status === 'solicitada' ? 'Solicitada pelo Admin' : 'Aguardando reenvio', tone: 'blue', detail: escapeHtml(a.doc_parecer_comentario || 'Aguardando proposta pedagógica da unidade.') })), '📄', 'blue'),
    ].join('');
  }
}
...
// A ação administrativa sempre é pontual: mesmo numa recorrência, o Admin decide
// somente sobre a data selecionada. A propagação fica restrita ao parecer pedagógico.
async function adminValidateOccurrence(id) {
  const trip = agenda.find((a) => a.id === id);
  if (!trip || currentUser?.role !== 'admin') return;
  ...
  if (!ok) return;
  await loadAgenda(); renderAgenda(); renderDashboard(); renderPendencias();
  toast('✅ Esta ocorrência foi validada. As demais datas da recorrência não foram alteradas.');
}
