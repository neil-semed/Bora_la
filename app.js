...
function applyRoleUI(role) {
  ...
  // definição da ferramenta que usa o markcarro"): mesmo botão único no
  // topo, só pro Condutor/Motorista (ver reportarPaneMecanicaBoraLa()).
  document.getElementById('btnPaneMecanicaBoraLa')?.classList.toggle('hidden', role !== 'motorista');
...
// PEDIDO DO USUÁRIO ("coloque botão de pane mecânica - usar a mesma
// definição da ferramenta que usa o markcarro"): mesma regra de disparo do
// reportarPaneMecanicaGeral() do MarkCarro (pages/painel-dia.js) - ao
// clicar agora, avisa os solicitantes das viagens de HOJE deste motorista
// cuja ida OU volta seja a partir de agora + 1 minuto, e manda 1 aviso
// consolidado pra gestão (admins). Uma única notificação por viagem
// afetada (não uma por ida e outra por volta).
async function reportarPaneMecanicaBoraLa() {
  if (currentUser?.role !== 'motorista') return;
  if (!confirm('Confirma o registro de pane mecânica? A gestão e os solicitantes das viagens de hoje ainda não realizadas serão avisados agora.')) return;

  const nomeMotorista = currentUser?.user_metadata?.full_name || currentUser?.full_name || currentUser?.email || 'Motorista';
  const agora = new Date();
  const limiar = new Date(agora.getTime() + 60000); // T + 1 minuto
  const hojeStr = fmtDate(new Date());

  try {
    const hojeViagens = getVisibleAgenda().filter((a) => a.trip_date === hojeStr && viagemConfirmadaParaMotorista(a));
    const afetadas = hojeViagens.filter((a) => {
      const saida = a.departure_time ? new Date(`${hojeStr}T${a.departure_time.substring(0, 5)}:00`) : null;
      const volta = a.return_time ? new Date(`${hojeStr}T${a.return_time.substring(0, 5)}:00`) : null;
      return (saida && saida >= limiar) || (volta && volta >= limiar);
    });

    await Promise.all(afetadas.map((a) => {
      const uid = requesterUserId(a);
      if (!uid) return Promise.resolve();
      return createNotification(
        uid,
        'Pane mecânica',
        `Sua viagem das ${horaComH(a.departure_time)} pode atrasar: pane mecânica reportada pelo motorista.`,
        a.id
      ).catch((e) => console.warn('Erro ao notificar solicitante afetado:', e));
    }));

    await notifyAdmins(
      'Pane mecânica reportada',
      `Pane mecânica reportada por ${nomeMotorista}.` + (afetadas.length ? ` ${afetadas.length} viagem(ns) de hoje a partir de agora foram avisadas.` : ' Nenhuma viagem a partir de agora pra avisar hoje.')
    );
    toast('✅ Pane registrada. Solicitante(s) e gestão foram avisados.');
  } catch (e) {
    console.error('Erro ao reportar pane mecânica:', e);
    toast('⚠️ Não foi possível registrar a pane agora. Tente de novo.', true);
  }
}
...
function renderDashboard() {
  ...
  // PEDIDO DO USUÁRIO ("acrescente o card de acumulado de viagens - carrega
  // somente do motorista logado"): total HISTÓRICO de viagens aprovadas/
  // confirmadas visíveis a este usuário (getVisibleAgenda() já limita ao
  // próprio motorista escalado, quando o perfil é motorista) - sem aplicar
  // os filtros de mês/data/unidade do topo (que são só pros cartões e
  // gráficos "no período").
  const acumulado = visibleBruto.filter((a) => a.situacao === 'aprovada' || a.situacao === 'confirmada').length;
  const statAcumuladoEl = document.getElementById('statAcumulado');
  if (statAcumuladoEl) statAcumuladoEl.textContent = acumulado;
...
function cardMarkCarroHTML(l, mostrarMotorista = false) {
  ...
  // cinza pequeno.
  // PEDIDO DO USUÁRIO ("pedi para sublinhar a palavra ida ou volta - não
  // falei pra tirar a formatação do texto que já estava E SUBLINHE A
  // PALAVRA"): mantém o destaque grande/negrito/azul de antes pra Ida/Volta
  // (não é mais só texto cinza pequeno) e ACRESCENTA o sublinhado na
  // palavra, além do sublinhado que já existia no horário correspondente.
  const classeRotulo = (destacarIda || destacarVolta) ? 'text-base font-bold text-blue-700' : 'text-slate-500 text-xs';
  const rotuloHorario = destacarIda ? '<u>Ida</u>' : destacarVolta ? '<u>Volta</u>' : 'Saída → Retorno';
  ...
  return `
    <div class="driver-trip-card rounded-lg overflow-hidden bg-white shadow-sm border-l-4 border-blue-400 mb-3" data-horario="${l.hora_saida || ''}">
      ...
      <div class="p-4 space-y-2.5 text-sm">
        ...
        ${l.justificativa ? `<div class="text-xs text-slate-600"><span class="text-slate-400">Justificativa:</span> ${escapeHtml(l.justificativa)}</div>` : ''}
      </div>
    </div>`;
}
