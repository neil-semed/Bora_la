...
create table if not exists profiles (
  ...
  full_name text,
  phone text,
  role text not null check (role in ('admin','escola','pedagogia','motorista','operacional','agente_externo')) default 'escola',
...
-- Perfis administrativos configuráveis (a definição completa e as políticas ficam
-- em migration_019_perfis_administrativos.sql para instalações já existentes).
...
create table if not exists access_profile_permissions (
  ...
  screen_key text not null check (screen_key in ('dashboard','agenda','solicitacao','relatorios','validacoes','km','veiculos','motoristas')), can_view boolean not null default false, can_edit boolean not null default false,
  primary key (access_profile_id, screen_key), check (not can_edit or can_view)
);
alter table profiles add column if not exists access_profile_id uuid references access_profiles(id) on delete set null;
...
create table if not exists cooperativas (
  ...
  phone text,
  active boolean not null default true,
  opera_atf boolean not null default true,
  opera_pcd boolean not null default false,
  created_at timestamptz default now()
);
...
create table if not exists excursions (
  ...
  passenger_access_status text not null default 'nao_liberado' check (passenger_access_status in ('nao_liberado','aberto','enviado','bloqueado')),
  passenger_access_submitted_at timestamptz,
  requester_name text,
  requester_address text,   -- só preenchido pra unidade "Outra" (não cadastrada)
  requester_contact text,   -- idem (telefone)
  requester_email text,     -- idem (e-mail)
  ...
  doc_status text not null default 'nao_enviado' check (doc_status in ('nao_enviado','solicitada','em_analise','correcoes','aceito','rejeitado')),
  ...
  -- Pendências: marca quando o Admin efetivamente tratou a ocorrência.
  admin_processed_at timestamptz,
  admin_processed_by uuid references profiles(id),

  -- Datas adicionais selecionadas no wizard de recorrência (as ocorrências
  -- correspondentes continuam em linhas próprias desta mesma tabela).
  selected_recurrence_dates jsonb not null default '[]'::jsonb
);
...
create table if not exists excursion_passengers (
  ...
  excursion_id uuid not null references excursions(id) on delete cascade,
  nome text not null,
  tipo_documento text not null default 'cpf' check (tipo_documento in ('cpf','rg','certidao_nascimento')),
  documento text,
...
--   motorista vê as próprias viagens e a Agenda Geral: apenas viagens confirmadas
--   que já tenham algum motorista atribuído (sem expor pedidos pendentes)
  ...
  using (
    ...
    or (public.current_role_name() = 'motorista' and (
          ...
          or (
            ...
            and exists (select 1 from excursion_drivers ed where ed.excursion_id = excursions.id)
          )
        ))
  );
