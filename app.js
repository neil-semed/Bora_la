...
function originName(trip) {
  ...
  if (trip.school_id) {
    return schoolName(trip.school_id);
  }

  return trip.requester_name || '—';
}


function originAddress(trip) {
  if (trip.origin_address) {
    return trip.origin_address;
  }

  if (trip.school_id) {
    return schoolAddress(trip.school_id) || '';
  }

  return trip.requester_address || '';
}
...
function renderAgendaPorDataItem(a) {
  ...
  ${originName(a)} → ${a.destination}
</div>

<div class="text-xs text-slate-500">
  ${hhmm(a.departure_time)} - ${hhmm(a.return_time)}
  • ${totalPax} passageiros
</div>
      </div>
...
function onWEscolaChange() {
  ...
  document
    .getElementById('wOutraBox')
    .classList.toggle('hidden', !isOutra);

  const tipo = document.querySelector(
    'input[name="wTipoSolicitacao"]:checked'
  )?.value || 'excursao';

  /*
   * No agendamento, a escola escolhida no Passo 1 também
   * será a escola de destino.
   */
  if (tipo === 'agendamento' && !isOutra && escolaId) {
    const destinoSelect = document.getElementById('wDestinoEscola');

    if (destinoSelect) {
      destinoSelect.value = escolaId;
...
function onWTipoSolicitacaoChange() {
  ...
  if (origemBox) {
    origemBox.classList.toggle('hidden', !isAgendamento);
  }

  if (destinoNormalBox) {
    destinoNormalBox.classList.toggle('hidden', isAgendamento);
  }

  if (destinoAgendamentoBox) {
    destinoAgendamentoBox.classList.toggle('hidden', !isAgendamento);
  }

  if (destinoTitulo) {
    destinoTitulo.textContent = isAgendamento
  ...
  if (isAgendamento) {
    ...
    if (
      escolaId &&
      escolaId !== '__outra__' &&
      document.getElementById('wDestinoEscola')
    ) {
      document.getElementById('wDestinoEscola').value = escolaId;
      onWDestinoEscolaChange();
    }
  }
}
...
function populateAgendamentoDestinoSelect() {
  ...
  const escolaSelecionada =
    document.getElementById('wEscola')?.value;

  if (
    escolaSelecionada &&
    escolaSelecionada !== '__outra__' &&
    visiveis.some((s) => s.id === escolaSelecionada)
  ) {
...
const tipoExcursao = document.querySelector(
  'input[name="wTipoSolicitacao"][value="excursao"]'
);

if (tipoExcursao) {
  tipoExcursao.checked = true;
}
...
function validateWizardStep(step) {
  ...
  if (step === 1) {
    ...
    /*
     * Para agendamento, a escola continua sendo obrigatória,
     * pois ela será o destino e também a unidade vinculada
     * à solicitação.
     */
    if (isAgendamento && (!escolaId || escolaId === '__outra__')) {
      toast(
        '⚠️ Selecione a escola que receberá a equipe/comitiva.',
        true
      );
      return false;
    }
    ...
    if (
      escolaId === '__outra__' &&
      !document.getElementById('wOutraNome').value.trim()
    ) {
      toast(
        '⚠️ Informe o nome da unidade/solicitante.',
        true
      );
      return false;
    }
    ...
    if (isAgendamento) {
      ...
      const origemNome =
        document.getElementById('wOrigemNome').value.trim();

      const origemEndereco =
        document.getElementById('wOrigemEndereco').value.trim();

      if (!origemNome) {
        toast(
          '⚠️ Informe o nome da origem.',
          true
        );
        return false;
      }
      ...
      if (!origemEndereco) {
        toast(
          '⚠️ Informe o endereço da origem.',
          true
        );
        return false;
      }
  ...
  if (step === 2) {
    ...
    if (isAgendamento) {
      ...
      const destinoEscola =
        document.getElementById('wDestinoEscola').value;

      if (!destinoEscola) {
        toast(
          '⚠️ Selecione a escola de destino.',
          true
        );
        return false;
      }
      ...
      const endereco =
        document.getElementById('wDestinoEscolaEndereco').value.trim();

      if (!endereco) {
        toast(
          '⚠️ A escola selecionada não possui endereço cadastrado.',
          true
        );
        return false;
      }
      ...
      if (
        !document.getElementById('wDestino').value.trim()
      ) {
        toast(
          '⚠️ Informe o destino.',
          true
  ...
  if (step === 3) {
    ...
    if (
      ...
      toast(
        '⚠️ Informe data e horário de saída.',
        true
      );
      return false;
    }
  ...
  if (step === 4) {
    ...
    if (
      ...
      toast(
        '⚠️ Escolha o público-alvo desta excursão.',
        true
      );
      return false;
    }
    ...
    if (
      ...
      toast(
        '⚠️ Informe o nome de cada aluno PCD (ou reduza a quantidade).',
        true
      );
      return false;
    }
    ...
    const recorrencia =
      document.querySelector(
        'input[name="wRecorrencia"]:checked'
      ).value;
    ...
    if (recorrencia !== 'unico') {
      ...
      if (!temDia || !fim) {
        toast(
          '⚠️ Escolha os dias da semana e a data final da recorrência.',
          true
        );
        return false;
      }
...
function renderResumo() {
  ...
  if (isAgendamento) {
    ...
    origem =
      document.getElementById('wOrigemNome').value || '-';

    origemEndereco =
      document.getElementById('wOrigemEndereco').value || '-';

    const destinoEscolaId =
      document.getElementById('wDestinoEscola').value;

    destino =
      schoolName(destinoEscolaId);

    destinoEndereco =
      document.getElementById('wDestinoEscolaEndereco').value || '-';

    cidade =
      document.getElementById('wCidadeAgendamento').value || '';

  } else {

    origem = escola;

    origemEndereco = isOutra
      ? (document.getElementById('wOutraEndereco').value || '-')
      : (schoolAddress(escolaId) || '-');
  ...
  const pca =
    wizardPcdList.length;

  const apoios =
    wizardPcdList.filter(
      (p) => p.nome_apoio && p.nome_apoio.trim()
    ).length;
  ...
  const turnoLabel =
    TURNO_LABELS[turnoFromHora(hora)] || '-';
  ...
  if (recorrencia !== 'unico') {
    ...
    recorrenciaTxt =
      `Continuado ${recorrencia} - ${
        dias || 'nenhum dia escolhido'
      }, até ${
        fim
          ? new Date(
              fim + 'T00:00'
  ...
  document.getElementById(
    ...
    <div class="text-slate-500">
      <strong>Endereço da origem:</strong>
      ${origemEndereco || '-'}
    </div>
    ...
    <div>
      <strong>Turno (automático):</strong>
      ${turnoLabel}
    </div>
    ...
    <div>
      ...
      ${
        pca
          ? ` + ${pca} PCD + ${apoios} apoio(s)`
          : ''
      }
      =
      <strong>
        ${alunos + acompanhantes + pca + apoios}
        pessoas
      </strong>
    </div>
  ...
  let sug =
    `<strong>🚌 Sugestão Automática de Veículos:</strong><br/>`;

  if (total === 0) {

    sug +=
      'Informe a quantidade de alunos para calcular.';

  } else {

    sug +=
      `Total: ${total} pessoas<br/>`;

    if (micros > 0) {
      sug +=
        `• ${micros} micro-ônibus (32 lugares)<br/>`;
    }

    if (vans > 0) {
      sug +=
        `• ${vans} van (15 lugares)<br/>`;
    }
    ...
    if (foraMunicipio) {
      sug +=
        `<br/>⚠️ Viagem FORA de Nova Lima - será necessária <strong>ATF</strong>`;
    }
  }
...
async function submitSolicitacao() {
  ...
  if (isAgendamento) {
    ...
    originName = isOutra
      ...
      : (
          schoolName(escolaId) || null
        );

    originAddress = isOutra
      ? (
          document.getElementById(
            'wOutraEndereco'
          ).value.trim() || null
  ...
  const base = {
     ...
     * IMPORTANTE:
     *
     * No agendamento, school_id continua sendo a escola
     * vinculada à solicitação — neste caso, a escola de destino.
     */
    school_id:
      isOutra
        ? null
  ...
  if (sb) {
    ...
    if (
      ...
      if (pcdErr) {
        ...
        toast(
          '⚠️ Viagem salva, mas houve erro ao salvar os alunos PCD: ' +
          pcdErr.message,
          true
        );
      }
    ...
    toast(
      novas.length > 1
        ? `✅ ${novas.length} viagens da recorrência foram criadas!`
    ...
    toast(
      novas.length > 1
        ? `✅ ${novas.length} viagens da recorrência foram criadas (modo demo)!`
        : '✅ Solicitação enviada (modo demo)!'
    );
  }
