...
// Origem real do transporte. Em solicitações antigas, usa a escola como fallback.
...
function origemFiltroEhOutro(trip) {
  if (trip && trip.solicitation_type === 'agendamento') return true;
  return !trip || !trip.school_id || schoolName(trip.school_id) === '—';
}
...
function viagemForaDeNovaLima(trip) {
  const cidade = String(trip?.city || '').trim().toLowerCase();
  return !!cidade && cidade !== 'nova lima';
}
...
async function confirmEnviarListagem() {
  ...
  const foraDeNovaLima = viagemForaDeNovaLima(trip);
  ...
  // REGRA ATF: toda viagem para fora de Nova Lima exige a relação completa de
...
function onWDestinoEscolaChange() {
  ...
  if (cidade && escola?.city) cidade.value = escola.city;
}
