...
create table if not exists public.access_profile_permissions (
  ...
  screen_key text not null check (screen_key in ('dashboard', 'agenda', 'solicitacao', 'relatorios')),
...
create policy excursions_operacional_select on public.excursions for select using (
  public.current_role_name() = 'operacional' and (
    ...
    public.has_access_permission('relatorios')
  )
);
...
begin
  ...
  if role_name = 'operacional' and public.has_access_permission('agenda', true) then return new; end if;
