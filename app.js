...
async function requestOwnPasswordReset() {
  ...
  if (!email) return;
  const { error } = await sb.auth.resetPasswordForEmail(email.trim(), { redirectTo: 'https://neil-semed.github.io/bora-la-excursoes/redefinir-senha.html' });
...
async function confirmAssign() {
  ...
  if (ok) {
    ...
    toast('✅ Viagem aprovada e motorista(s) atribuído(s)!' + (precisaLista ? ' A listagem de passageiros foi liberada.' : ''));
  }
  closeAssignModal();
}
