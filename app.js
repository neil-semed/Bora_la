...
const SCREEN_ROLES = {
  ...
  perfisacesso: ['admin'],
  validadores: ['admin'],
  relatorios: ['admin', 'escola', 'pedagogia', 'motorista', 'agente_externo'],
...
const ROLE_LABELS = { admin: 'Admin', escola: 'Escola', pedagogia: 'Pedagogia', motorista: 'Motorista', operacional: 'Administrativo', agente_externo: 'Agente Externo' };
...
function renderPendencias() {
  ...
  if (ehAdmin) {
    ...
    container.innerHTML = [
      pendenciaBloco('Solicitações ainda não verificadas', 'Em ordem de criação; indicador sem ações administrativas.', naoVerificadas.map((a) => pendenciaCard(a, { badge: 'Aguardando Pedagogia', tone: 'amber', action: botaoAgenda(a) })), '📥', 'amber'),
      pendenciaBloco('Sem validação pedagógica', 'Solicitações pendentes de parecer pedagógico.', semValidacao.map((a) => pendenciaCard(a, { badge: 'Sem validação', tone: 'amber', action: botaoAgenda(a) })), '⏳', 'amber'),
      pendenciaBloco('Proposta aguardando da unidade', 'Pedido do Admin ou correção solicitada pela Pedagogia.', reenvio.map((a) => pendenciaCard(a, { badge: a.doc_status === 'solicitada' ? 'Solicitada pelo Admin' : 'Aguardando reenvio', tone: 'blue', detail: escapeHtml(a.doc_parecer_comentario || 'Aguardando proposta pedagógica da unidade.'), action: botaoAgenda(a) })), '📄', 'blue'),
    ].join('');
  }
}
