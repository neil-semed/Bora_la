...
async function renderFinanceRequests() {
  ...
  if (!box || currentUser?.role !== 'financeiro') return;
  ...
  const linhas = filtered
    ...
    .map((a) => {
      ...
      if (a.status === 'pending' && isAgendaEditor) {
        const validarTexto = a.recurrence_group_id ? 'Validar recorrência' : 'Aprovar (Pedagogia)';
        acoes = `
          <button onclick="pedagogyApprove('${a.id}')" class="text-emerald-600 hover:text-emerald-800 text-xs font-medium mr-3">${validarTexto}</button>
...
function renderPendencias() {
  ...
  const acoesAdmin = (a) => `<div class="flex flex-wrap gap-2">${botaoAgenda(a)}${a.doc_status === 'nao_enviado' ? `<button onclick="solicitarPropostaPedagogica('${a.id}')" class="shrink-0 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm font-bold text-blue-700 hover:bg-blue-100">Solicitar proposta</button>` : ''}</div>`;
  ...
  if (ehAdmin) {
    ...
    container.innerHTML = [
      pendenciaBloco('Novas solicitações', 'Aguardando a primeira tratativa do Admin.', novas.map((a) => pendenciaCard(a, { badge: 'Nova solicitação', tone: 'emerald', action: acoesAdmin(a) })), '🚌', 'emerald'),
      pendenciaBloco('Sem validação pedagógica', 'Solicitações que ainda não receberam parecer pedagógico.', semValidacao.map((a) => pendenciaCard(a, { badge: 'Aguardando validação', tone: 'amber', action: acoesAdmin(a) })), '⏳', 'amber'),
      pendenciaBloco('Proposta pedagógica aguardada', 'Solicitada pelo Admin ou devolvida pela Pedagogia.', reenvio.map((a) => pendenciaCard(a, { badge: a.doc_status === 'solicitada' ? 'Solicitada à unidade' : 'Aguardando reenvio', tone: 'blue', detail: escapeHtml(a.doc_parecer_comentario || 'Aguardando proposta pedagógica da unidade.'), action: acoesAdmin(a) })), '📄', 'blue'),
    ...
    const listagens = minhas.filter((a) => viagemValidadaCompletamente(a) && precisaCooperativa(a) && ['nao_enviada', 'rejeitada'].includes(a.listagem_status || 'nao_enviada'));
    ...
    container.innerHTML = [
      pendenciaBloco('Proposta pedagógica', 'Envie ou reenvie o arquivo solicitado pelo fluxo administrativo/pedagógico.', propostas.map((a) => pendenciaCard(a, { badge: a.doc_status === 'correcoes' ? 'Correções solicitadas' : 'Proposta solicitada', tone: 'blue', detail: escapeHtml(a.doc_parecer_comentario || 'Envie a proposta pedagógica para a continuidade do fluxo.'), action: botaoProposta(a) })), '📄', 'blue'),
      pendenciaBloco('Listagem de passageiros', 'Viagens aprovadas que exigem lista por ATF e/ou PCD.', listagens.map((a) => pendenciaCard(a, { badge: a.listagem_status === 'rejeitada' ? 'Listagem para corrigir' : 'Listagem para enviar', tone: 'emerald', action: botaoLista(a) })), '👥', 'emerald'),
      pendenciaBloco('Transporte PCD confirmado', 'Informação recebida da cooperativa responsável pelo transporte adaptado.', pcdConfirmado.map((a) => pendenciaCard(a, { badge: 'PCD confirmado', tone: 'blue', detail: escapeHtml([a.pcd_cooperativa_veiculo && `Veículo: ${a.pcd_cooperativa_veiculo}`, a.pcd_cooperativa_motorista && `Motorista: ${a.pcd_cooperativa_motorista}`].filter(Boolean).join(' · ') || 'A cooperativa confirmou o atendimento PCD.') })), '♿', 'blue'),
    ].join('');
  } else {
    ...
    container.innerHTML = [
      pendenciaBloco('Solicitações sem validação pedagógica', 'Em ordem de criação; aguardando parecer da Pedagogia.', semValidacao.map((a) => pendenciaCard(a, { badge: 'Aguardando Pedagogia', tone: 'amber', action: botaoValidacao(a) })), '⏳', 'amber'),
      pendenciaBloco('Proposta aguardando da unidade', 'Pedido do Admin ou correção solicitada pela Pedagogia.', reenvio.map((a) => pendenciaCard(a, { badge: a.doc_status === 'solicitada' ? 'Solicitada pelo Admin' : 'Aguardando reenvio', tone: 'blue', detail: escapeHtml(a.doc_parecer_comentario || 'Aguardando proposta pedagógica da unidade.'), action: botaoValidacao(a) })), '📄', 'blue'),
    ].join('');
  }
}
...
function renderValidacoesEscola() {
  ...
  tbody.innerHTML = minhas
    ...
    .map((a) => {
      const podeEnviar = a.status === 'pending' && ['nao_enviado', 'solicitada', 'correcoes'].includes(a.doc_status);
...
function renderValidacoesPedagogia() {
  ...
  let lista = agenda.filter((a) => a.status === 'pending' && a.doc_status !== 'solicitada');
...
function financeItemsFromWizard() {
  ...
  }).filter((item) => item.description || item.quantity || item.value);
}
...
function renderVeiculos() {
  ...
  grid.innerHTML = vehicles.map((v) => `
    <div class="bg-white rounded-xl border border-slate-200 p-5 card-hover">
      ...
      ${(() => { const d = drivers.find((item) => item.vehicle_id === v.id); return d ? `<div class="mt-2 mb-3 rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-700"><strong>Motorista:</strong> ${escapeHtml(d.name)}<br><strong>Telefone:</strong> ${escapeHtml(d.phone || 'não informado')}</div>` : '<p class="mb-3 text-xs text-slate-400">Sem motorista vinculado.</p>'; })()}
