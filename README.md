...
- Toda notificação vinculada a uma viagem mostra a data, o horário de saída e o horário de retorno.
- O status ATF **Não Precisa** é exibido em azul na Agenda Mestra.
- A aprovação administrativa sem motorista deixa a viagem em **Aguarda motorista** sem violar a restrição do banco; após aprovar ou reprovar, o card sai de **Novas solicitações**.

Depois de publicar os arquivos no GitHub, execute `supabase/migration_028_notificacoes_agenda_situacao.sql` no **SQL Editor** do Supabase. Sem essa etapa, o erro `excursions_situacao_check` continuará ocorrendo.

## Atualização 27 — botão Hoje, filtros e envio de listagem

- A Agenda Mestra ganhou o botão **Hoje**, que seleciona a data local atual.
- O filtro por tipo da tela **Pendências** agora também funciona para Escola e Pedagogia, e o Admin pode filtrar listagens aguardando conferência.
- A Escola pode enviar ou reenviar a listagem de uma viagem vinculada à sua unidade mesmo quando a solicitação foi cadastrada pelo Admin.

Depois de publicar os arquivos no GitHub, abra o painel do Supabase, entre em **SQL Editor**, copie todo o conteúdo de `supabase/migration_027_corrige_listagem_escola.sql` e clique em **Run**. Sem essa etapa, a correção da listagem não entra em vigor.
...
### Novidades desta versão: filtro de Unidade retirado do Dashboard do motorista

Você pediu pra conferir que o Dashboard do motorista (a tela "📊 Dashboard" que ele também tem no menu, além de
"Minhas Viagens") mostrasse só as informações das viagens dele mesmo, nos cartões de resumo e nos dois
gráficos, e que o filtro "Unidade" fosse retirado de lá.

- Os cartões (Viagens Hoje, Pendentes, Aprovadas, Passageiros, Canceladas, Desaprovadas) e os dois gráficos
  ("Viagens por mês" e "Viagens por unidade solicitante") **já vinham calculados só com as viagens atribuídas
  ao motorista** - conferi isso rodando o Dashboard como motorista e comparando com o do admin (que vê tudo).
  Não havia nada de errado aí, era assim desde que o Dashboard foi criado.
- O que realmente fazia sentido tirar era o filtro **"Unidade"**: ele serve pra quem acompanha viagens de
  várias unidades ao mesmo tempo (admin/pedagogia) escolher uma só pra olhar - pro motorista, que já só vê as
  próprias viagens, esse filtro não ajudava em nada. Ele já tinha sido escondido antes pro perfil Escola (pelo
  mesmo motivo); agora o motorista entra nesse mesmo tratamento.
- Nenhuma migração de banco foi necessária - é um ajuste só de JavaScript.
...
**Forma recomendada agora:** depois de enviar a pasta `.github` e a pasta `android` deste
projeto para o GitHub, abra **Actions** → **Gerar APK do Motorista** → **Run workflow**.
Ao terminar, baixe o artefato `Bora-La-Motorista-APK`. O passo a passo completo está em
`android/README.md`. Esta opção não depende do navegador nem do PWABuilder.
...
## Atualização de segurança obrigatória (versão 014)

Para instalações já existentes, execute **uma única vez** o arquivo
`supabase/migration_014_security_hardening.sql` no SQL Editor do Supabase, depois
da migration 013. Ele impede autoelevação de privilégios, limita as alterações que
Escola/Pedagogia/Motorista podem fazer em uma viagem e expira os links públicos de
passageiros em sete dias.

Depois publique novamente o conteúdo de `google-apps-script/Code.gs`, seguindo o
passo a passo atualizado em `google-apps-script/README.md`. Essa publicação agora
exige as propriedades `SUPABASE_URL` e `SUPABASE_ANON_KEY` no Apps Script e requer
Google Workspace para manter os documentos fora da internet pública.
...
### Fluxo de passageiros e notificações (atualização)
- Rode no Supabase a migration `supabase/migration_013_notificacoes_fluxo_passageiros.sql`.
- A lista de passageiros é liberada somente depois da aprovação e atribuição de motorista.
- Escola preenche em **Validações**; entidade/comitiva usa o link externo enviado por e-mail.
- A quantidade pode ser menor que a capacidade; nunca pode ultrapassar a capacidade dos veículos atribuídos.
- Em viagens com origem ou destino fora de Nova Lima, nome completo, tipo de documento e número são obrigatórios.
- O sino de notificações usa a tabela `notifications`.
- O e-mail automático usa a Edge Function `send-cooperativa-email` e a chave `RESEND_API_KEY` já prevista no projeto.
# Lembrete de 24 horas para motorista

Após executar `supabase/migration_016_lembrete_motorista.sql`, publique a função:

```bash
supabase functions deploy send-driver-reminders
```

Configure um agendador externo ou Supabase Cron para chamar a função a cada hora com o cabeçalho `x-cron-secret`. Cadastre o mesmo valor como secret `CRON_SECRET` da função. Ela envia uma única notificação interna aos motoristas escalados, cerca de 24 horas antes de cada viagem confirmada.
