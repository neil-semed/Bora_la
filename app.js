...
async function loadProfiles() {
  if (sb) {
    const { data } = await sb.from('profiles').select('id, full_name, email, phone, role, school_id, driver_id, active');
    profilesById = {};
    allProfiles = data || [];
...
async function confirmCreateUser() {
  ...
  try {
    ...
    const resp = await fetch(`${url}/functions/v1/admin-create-user`, {
      ...
      body: JSON.stringify({ email, password, full_name: full_name || null, phone, role, school_id, driver_id, setor_pedagogico }),
    });
...
function showScreen(name, el) {
  ...
  const titles = {
    ...
    agenda: isMotoristaAgenda
      ? ['Minhas Viagens', `Viagens atribuídas a ${currentUser?.user_metadata?.full_name || currentUser?.full_name || currentUser?.email || 'você'}`]
